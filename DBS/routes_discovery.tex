...
chunks that really are lost will be requested \leorem{no entiendo para
  que se piden}. {\color{red} Se piden aquellos chunks que no han sido
...
peer of the team\leorem{solo uno? cuando se produce pruning?}
...
the request is ignored \leorem{y cómo se recupera?} {\color{red} Si el
...
maximum number of generated duplicates \leo{at requester's side}), the
corresponding entry in the $\text{forward}[]$ table is deleted
\leorem{Es por chunk o por origen?} {\color{red} Los duplicados son de
...
the neighborhood degree of $P_i$ for the origin $P_j$ is 5\leorem{yo
  definiría dos tipos de grado de vecindad, el mio y el de reenvio}
