...
chunks that really are lost will be requested \leo{\{no entiendo para
  que se piden si $p_h=0$, es decir, se ha llegado al $p_p$}.  
...
the request is ignored). \leo{This produces peer $P_i$ continues forwarding $P_x$'s chunks to $P_j$}. 

\leo{Due to the random selection of the} request messages, redundant routes can be created. Therefore, some chunks could be received more than once. This is an overhead to be minimized. A peer send a pruning message to a peer \leo{that sends a duplicated chunk}. The receiver of
...
\leo{\hl{Leo:} No bastaría con quitar el que envía el prunning del forward para ese origen?}
...
neighbor peer, for that origin \leo{(it can be itself)}. 

The continued reception of requesting and pruning messages at peer $P_i$ produces the list
$\text{forward}[P_i]$ gets shorter \leo{because chunks having $P_i$ as origin reach other peer sooner from a requested peer than from $P_i$} (smaller \gls{neighborhood-degree})
