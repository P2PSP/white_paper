...
round-robing schema, where it can be up to $N^*$ \emph{peers}. Each
peer gathers the chunks from the splitter and the rest of peers of the
team, and sends them to a \emph{player}\footnote{Peer should run even
  if no player(s) are connected to it.}. In
Tab.~\ref{tab:DBS_nomenclature} the definitions used in the DBS have
been summarized.
\begin{comment}
