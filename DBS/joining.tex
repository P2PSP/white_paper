...
list\footnote{Peers also updates their team list every time a chunk is
  ...
  chunk.}  and to a \leochange{table}{jagged array} of peers called $\mathtt{forward}[]$
\note{(see
...
\begin{figure*}[h!]
  ...
  \subfloat[A full-connected overlay.]{\label{fig:full_mesh}
    \vbox{\myfig{graphics/full-mesh}{6cm}{400}}}
  \quad
  \subfloat[A star-shaped overlay.]{\label{fig:star}
    \vbox{\myfig{graphics/star}{6cm}{400}}}
...
incoming peer to the list and \leo{(4) send a chunk to each peer in its list}. Notice that only those peers that are in
