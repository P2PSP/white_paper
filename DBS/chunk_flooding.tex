...
neighbors. Even controlling the chunk rate by the servers, in order to
reduce network congestion while peers's flooding, some kind of flow
control must be performed.

%Moreover, to achieve an ideal I/O ratio of $1$, peers should send one
%chunk for every received one.

Congestion may be avoided by means of a basic idea: \emph{if I have
...
\begin{figure}
  \imgw{250}{graphics/team_1.svg} \caption{At $t_2$, peer $P_1$ joins
...
\begin{figure}
  \imgw{300}{graphics/team_2.svg} \caption{$P_2$ joins the team. $M_0$
