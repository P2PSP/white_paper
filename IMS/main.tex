...
(Virtual LAN)s~\cite{shabtay2011ip}, but not in the
Internet~\cite{}. IMS runs on the top of DBS and provides efficient
native IPM, where available.

The idea of IMS is simple: all peers in the same LAN or VLAN have the
same network address. When a joining peer ${\cal P}^j_i$ receives
$[{\cal T}^j]$ from ${\cal S}^j$, first checks if there are neighbors
in the same subnet (all of them share the same private network
address). For all those peers, ${\cal P}^j_i$ uses the IP address
$\mathtt{224.0.0.1}$ (all systems on this subnet), (default) port
$\mathtt{1234}$, to broadcast (only) the chunks received from the
splitter. When implementing IMS, all peers in the same local network
communicate using this IPM group and port. The splitter do a similar
check: for all those peers that are in the same local network that
then splitter, the splitter sends the chunk to the multicast channel
$\mathtt{224.0.0.1}$:$\mathtt{1234}$.
