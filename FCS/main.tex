...
\leochange{connectivity}{upload capacity}) can stay in the team thanks to the generosity of the
...
\item The $\mathtt{pending}[]$ table is run in the order provided by
  ...
  debts.
\item The \leo{ list $\mathtt{debt}[]$ and therefore the} run of $\mathtt{pending}[]$ is \leochange{reset}{updated} in each round (when a
...
\item In DBS, request messages are sent selecting the destination peers
  ...
  higher debt \leorem{a los que he enviado y no me han enviado? Debería ser al revés}. Thus, if the \leochange{insolidarity}{lack of solidarity} is produced by an overlay
  topology imbalance (an extreme example is in Fig.~\ref{fig:star}),
  \leochange{badly connected peers peers}{peers poorly connected} could have the chance of mitigating this
  problem by forwarding more chunks to their neighbors. \leorem{Me lo explique mejor}.
\end{enumerate}
