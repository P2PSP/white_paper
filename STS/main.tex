...
\begin{figure}
  ...
  hybrid CDN(Icecast)/P2PSP network. $S$ represents a splitter and $P_i$ a
...
seen, a listener (which usually plays de stream) can be replaced by
a \emph{splitter}, a P2PSP entity that sends the received stream (a
...
CDN/P2PSP structures. When a P2PSP user (which knows an URL of the
channel) runs a local peer, it provides to the peer with the URL of
the channel: a server and a mount point. Then, the peer (as any other
listener does) contacts with a load balancer which in this case sends
...
URL of a channel) contacts with the load balancer which returns a list
...
successfull connection is finally selected.\footnote{In this case, the
  load balancer should provide to the splitter if each entry of the
  list is a server or a peer, in order to prioritize the type of
  source.}

Finally, using the idea of the extended load balancer, when a player
