...
balancer redirects it towards the local peer.\footnote{Obviously, to
  establish a connection, both, the player and the local peer must
  reference the same channel. Otherwise, the peer will remain asleep
  waiting for a HTTP GET request and the player will be redirected to
  a different server.}
...
generated when a new splitter is instantiated. This entity keeps
updated a table indexed by names of channels, where every entry
contains a list of splitters transmiting the corresponding
