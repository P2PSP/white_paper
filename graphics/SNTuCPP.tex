...
\begin{tikzpicture}
  ...
  % Entities
  \node[] (Peer A) {Peer $P_1$};
  \node[right=3 of Peer A] (NAT A) {NAT $\cal{A}$};
  \node[right=3 of NAT A] (Monitor) {Monitor host $\cal{M}$};
  \node[right=2.5 of Monitor] (Splitter) {Splitter host $\cal{S}$};
  \node[right=3 of Splitter] (NAT B) {NAT $\cal{B}$};
  ...
  \node (S00) at ($(Splitter) + (0,-3ex)$) [fill=white!100,draw,rounded corners] {$\mathsf{S}$};
  \node (M00) at ($(Monitor) + (0,-3ex)$) [fill=white!100,draw,rounded corners] {$\mathsf{M}$};
  \draw[myptr] (S00) -- (M00) node [pos=0.5,fill=white!100] {\{\}};
  
  % 01
  \node (01) at ($(00) + (0,-3ex)$) {01};
  \path let \p{Splitter}=(Splitter),\p{01}=(01) in node (S01) at (\x{Splitter},\y{01}) [fill=white!100,draw,rounded corners] {$\mathsf{S}$};
  \path let \p{NAT A}=(NAT A),\p{01}=(01) in node (NATA01) at (\x{NAT A},\y{01}) [fill=white!100,draw,rounded corners] {$\mathsf{A0}$};
  \draw[myptr] (S01) -- (NATA01) node [pos=0.75,fill=white!100] {$\{M=(\cal{M},\mathsf{M})\}$};
  ...
  \draw[myptr] (NA02) -- (PA02) node [pos=0.5,fill=white!100] {$\{M=(\cal{M},\mathsf{M})\}$};
